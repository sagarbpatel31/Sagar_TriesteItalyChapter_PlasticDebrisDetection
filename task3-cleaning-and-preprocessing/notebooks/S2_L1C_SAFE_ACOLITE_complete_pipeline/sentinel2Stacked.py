...
def makeStackAndAngles(cmdargs):
    ...
    (fd, anglesfile) = tempfile.mkstemp(dir=cmdargs.tempdir, prefix="angles_tmp_", suffix=".img")
    os.close(fd)
    ...
    # Make a stack of the reflectance bands
    bandList = ['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A',
                'B09', 'B10', 'B11', 'B12']
    imgDir = f"{cmdargs.granuledir}/IMG_DATA"
    resampledBands = []

    for band in bandList:
        (fd, tmpBand) = tempfile.mkstemp(dir=cmdargs.tempdir, prefix=f"tmp_{band}_", suffix=".vrt")
        os.close(fd)
        inBandImgList = glob.glob(f"{imgDir}/*_{band}.jp2")
        if len(inBandImgList) != 1:
            raise fmaskerrors.FmaskFileError(f"Cannot find input band {band}")
    ...
    # Create output stack as GeoTIFF
    (fd, tmpStack) = tempfile.mkstemp(dir=cmdargs.tempdir, prefix="tmp_allbands_", suffix=".tif")
    os.close(fd)
    os.remove(tmpStack)  # Important: delete it so gdal_merge can recreate it
    cmdargs.toa = tmpStack

    if cmdargs.verbose:
        print(f"Making stack of all bands, at {cmdargs.pixsize}m pixel size")

    usingExceptions = gdal.GetUseExceptions()
    import subprocess
    merge_cmd = ['gdal_merge.py', '-q', '-of', 'GTiff', '-separate', '-o', cmdargs.toa] + resampledBands
